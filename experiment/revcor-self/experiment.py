...
# generate trial files: only one actor 1, which is the participant's own face
# 1 practice block, then (n_blocks + (repeat_last_block==True)?1:0) blocks of n_stims trials
n_actors = 1 # sampling from actors 1..n_actors
actors = [subject_number]  # only one actor, which corresponds to the participant's own face
